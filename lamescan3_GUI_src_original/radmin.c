...
#include "radmin.h"
#undef CONST //2fish lame lib fix
...
int radmin_reply_challenge(void *dst, void *src, char *pass)
{
	int i;
	char *p;
	char hash[16];
...
int radmin_auth2(tcp_open_struc *host, char *password, int timeout)
{
	...
	if((conn = tcp_open_indirect(host)))
	{
		...
		if(radmin_send_packet_ver1(conn, &request, timeout))
		{
			if(radmin_recv_packet_ver1(conn, &challenge, timeout))
			{
				...
				{
					if(radmin_reply_challenge(resp, challenge.data, password))
					{
						response.code = 0x09;
						response.data = resp;
						response.datalen = 32;
